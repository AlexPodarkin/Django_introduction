...
def index_init(request):
    ...
    return HttpResponse('<h3>Lection 3</h3>')


class HelloView(View):
    def get(self, request):
        return HttpResponse('<h2>Hello from class!</h2>')


def year_post(request, year):
    text = f'формируем статьи за {year} год'
    return HttpResponse(f'{text}...')


class MonthPost(View):
    def get(self, request, year, month):
        text = f'формируем статьи за {year} год и {month} месяц'
        return HttpResponse(f'{text}...')
